	...
	<main style="width: auto; height: auto;">
		...
		<div
			...
			<div
				...
				<h2
					style="font-size: 33px; text-align: left; width: 200px; margin-top: 20px; color: #000099; font-weight: bold;">Thanh Toán</h2>
					<hr style="background-color: #cc6600; height: 1.5px">
            <div class="">
            		
                    <div style="display: flex;">
                    	<div style="display: flex;flex-direction: column;">
                    	<h4 style="text-align: left; width: 300px;color: #000099;">Thông tin thanh toán</h4>
          <c:if test="${not empty cart.carts}">
            <c:set var="cartItems" value="${cart.carts}" scope="session"/>
            <c:set var="totalAmount" value="${cart.orderTotal}" scope="session"/>
        </c:if>
                <form action="PayServlet" method="post">
                    <div class="form-group">
                        <label for="inputName">Họ và tên</label>
                        <input type="text" class="form-control" id="inputName" name="fullName" placeholder="Họ tên đầy đủ" required>
                    </div>
	                    <div class="form-group">
	                        <label for="inputEmail">Email</label>
	                        <input  type="email" class="form-control" id="inputEmail" name="email" placeholder="...@gmail.com" required>
	                    </div>
	                    
	                    <div class="form-group">
                        <label for="paymentMethod">Hình thức thanh toán</label>
                        <select class="form-control" id="paymentMethod" name="paymentMethod" style="width: 400px;" required>
                            <option value="Nhận hàng trước khi thanh toán">Nhận hàng trước khi thanh toán</option>
                            <option value="Thanh toán chuyển khoản trước">Thanh toán chuyển khoản trước</option>
                        </select>
                    </div>
	                    	</div>
                    	...
                    	<h4 style="text-align: left; width: 300px;color: #000099;">Thông tin giao hàng</h4>
                    <div class="form-group">
                        <label for="inputAddress">Địa chỉ giao hàng</label>
                        <input type="text" class="form-control" id="inputAddress" name="address" placeholder="Ghi ghi số nhà.." required>
                    </div>
                    <div class="form-group">
                        <label for="inputPhone">Số điện thoại </label>
                        <input type="tel" class="form-control" id="inputPhone" name="phone" placeholder="Liên hệ khi gọi giao hàng..." required>
                    </div>
                     <div class="form-group">
                        <label for="ghichu">Ghi chú</label>  
                        <textarea style="width: 500px" class="form-control" rows="" cols="" id="ghichu" name="ghichu"  placeholder="Ghi chú ý về đơn hàng , ví dụ thời gian lấy hàng hay chỉ dẫn địa điểm giao hàng chi tiết hơn..."></textarea>
                    </div>
                    	</div>
                    </div>
                    
                    <h4 style="text-align: left; width: 300px;color: #000099;">Thông tin mua hàng</h4>
                        ...
                        <tbody>
                            <c:forEach var="item" items="${cart.carts}" varStatus="counter">
                                <tr style="font-size: 20px; color: #666666; text-align: center;">
                                  	...
                                  	<td>${item.tensanpham} <i style="color: red" class="fas fa-times">  ${item.soluongmua}</i></td>
                            ...
                            <tr>
								<td colspan="1"><h5 style="color: black; font-weight: bold;">Tạm tính</h5></td>
								<td colspan="2"><h5 style="color: black; font-weight: bold;"><fmt:formatNumber value="${cart.orderTotal}" type="currency" currencySymbol="₫" /></td>
						
							</tr>
							  <tr>
								<td colspan="1"><h5 style="color: black; font-weight: bold;">Giao hàng</h5></td>
								<td colspan="2"><h5 style="color: black; font-weight: bold;">Giao hàng miễn phí.</td>
							</tr>
							 <tr>
								<td colspan="1"><h4 style="color: black; font-weight: bold;">Tổng tiền</h4></td>
								<td colspan="2"><h4 style="color: black; font-weight: bold;"><fmt:formatNumber value="${cart.orderTotal}" type="currency" currencySymbol="₫" />
								<input type="hidden" id="totalAmount" name="totalAmount" value="${cart.orderTotal}$" readonly></td>
							</tr>
                        </tbody>
                    </table>
                    <input type="hidden" name="action" value="processPayment">
                    <input type="submit" class="btn btn-primary" value="Xác Nhận Thanh Toán" style=" width: 400px; height: 50px; 
						font-size: 20px; font-weight: bold; background-color:  #995c00; color: white; border: none;border-radius: 3px ">
